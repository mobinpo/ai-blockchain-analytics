...
// Enhanced sentiment update with proper error handling
const updateSentimentData = async () => {
  try {
    ...
    // Simulate API call failure (10% chance)
    if (Math.random() < 0.1) {
    ...
    // Randomly update sentiment values slightly for demo effect
    ...
    trendingTopics.value.forEach(topic => {
      if (Math.random() > 0.7) { // 30% chance to update
        ...
        topic.mentions = Math.max(100, topic.mentions); // Keep minimum
...
<template>
  <AppLayout>
    <!-- Header -->
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-900 mb-2">Sentiment Analysis</h1>
      <p class="text-gray-600">Real-time market sentiment tracking across social media and news platforms</p>
    </div>
